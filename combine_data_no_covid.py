...
def combine_weekly_data_no_covid(overlap_only=False):
    """
    Combine ICSA, gold, and crypto weekly data into a single dataset, excluding 2020-2021.
    ...
    print("Loading data files (excluding COVID period 2020-2021)...")
    ...
    icsa_df = icsa_df[
        (icsa_df['observation_date'].dt.year < 2020) | 
        (icsa_df['observation_date'].dt.year > 2021)
    ]
    ...
    print(f"ICSA data (no COVID): {len(icsa_df)} weeks")
    ...
    # Filter out 2020 and 2021 data from gold
    gold_df['week_ending_date'] = pd.to_datetime(gold_df['week_ending_date'])
    gold_df = gold_df[
        (gold_df['week_ending_date'].dt.year < 2020) | 
        (gold_df['week_ending_date'].dt.year > 2021)
    ]
    gold_df['week_ending_date'] = gold_df['week_ending_date'].dt.strftime('%Y-%m-%d')
    
    print(f"\nGold data (no COVID): {len(gold_df)} weeks")
    ...
    # Filter out 2020 and 2021 data from crypto
    crypto_df['week_ending_date'] = pd.to_datetime(crypto_df['week_ending_date'])
    crypto_df = crypto_df[
        (crypto_df['week_ending_date'].dt.year < 2020) | 
        (crypto_df['week_ending_date'].dt.year > 2021)
    ]
    crypto_df['week_ending_date'] = crypto_df['week_ending_date'].dt.strftime('%Y-%m-%d')
    ...
    print(f"\nCrypto data (no COVID): {len(crypto_df)} weeks")
    ...
    if overlap_only:
        output_file = os.path.join(output_dir, "final_data_overlap_only_no_covid.csv")
        ...
        print(f"\nFiltered to overlap-only data (no COVID): {len(combined_df)} weeks")
    else:
        output_file = os.path.join(output_dir, "final_data_no_covid.csv")
    
    combined_df.to_csv(output_file, index=False)
    print(f"\nCombined data saved to: {output_file}")
    
    # Display summary
    print(f"\nCombined dataset (no COVID): {len(combined_df)} weeks")
    ...
    if len(overlap_df) > 0:
        print(f"\nOverlapping period with all data available (no COVID):")
...
def create_both_datasets_no_covid():
    """Create both full and overlap-only datasets without COVID period"""
    print("=== Creating Full Dataset (with missing data, no COVID) ===")
    full_df = combine_weekly_data_no_covid(overlap_only=False)
    
    print("\n=== Creating Overlap-Only Dataset (complete data only, no COVID) ===")
    overlap_df = combine_weekly_data_no_covid(overlap_only=True)
    
    return full_df, overlap_df

if __name__ == "__main__":
    create_both_datasets_no_covid()
